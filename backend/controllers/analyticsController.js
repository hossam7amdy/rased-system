         ...
         ORDER BY attendance_percentage ASC`,
			...
			res.json({
				...
				data: {
					...
					statistics: {
						total_sessions: parseInt(stats.total_sessions, 10),
						total_students: parseInt(stats.total_students, 10),
						total_attendance: parseInt(stats.total_attendance_records, 10),
						average_attendance:
							parseInt(stats.total_sessions, 10) > 0
								? Math.round(
										(parseInt(stats.total_attendance_records, 10) /
											(parseInt(stats.total_sessions, 10) *
												parseInt(stats.total_students, 10))) *
											100,
									)
								: 0,
					},
			...
			res.json({
				...
				data: {
					statistics: {
						...
						total_sessions: parseInt(stats.total_sessions, 10),
						attended_sessions: parseInt(stats.attended_sessions, 10),
						overall_percentage:
							parseInt(stats.total_sessions, 10) > 0
								? Math.round(
										(parseInt(stats.attended_sessions, 10) /
											parseInt(stats.total_sessions, 10)) *
											100,
									)
								: 0,
	...
	exportAttendance: async (req, res) => {
		try {
			...
			if (sessionId) {
				...
				const _session = sessionResult.rows[0];
