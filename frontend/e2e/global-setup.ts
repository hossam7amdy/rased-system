...
export const API_BASE = process.env.PLAYWRIGHT_API_BASE ?? "http://localhost:5000/api";

export const CREDS = {
	admin: { email: "admin@rased.edu", password: "admin123", path: "/admin" },
	professor: { email: "prof@rased.edu", password: "prof123", path: "/professor" },
...
export default async function globalSetup(_config: FullConfig) {
	...
	const ping = await fetch(`${API_BASE}/auth/login`, {
		...
		body: JSON.stringify({ email: CREDS.admin.email, password: CREDS.admin.password }),
	});
	...
	await Promise.all([
		saveAuthState(CREDS.admin.email, CREDS.admin.password, CREDS.admin.path, path.join(authDir, "admin.json")),
		saveAuthState(CREDS.professor.email, CREDS.professor.password, CREDS.professor.path, path.join(authDir, "professor.json")),
		saveAuthState(CREDS.student.email, CREDS.student.password, CREDS.student.path, path.join(authDir, "student.json")),
	]);
}
