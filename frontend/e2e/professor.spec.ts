...
import { professorTest as test } from "./fixtures/auth";
...
test.describe("Professor dashboard", () => {
	...
	test("courses dashboard renders", async ({ page }) => {
		await expect(page.getByRole("button", { name: "المواد الدراسية" }).first()).toBeVisible();
	});
	...
	test("starts an attendance QR session", async ({ page }) => {
		...
		const startBtn = page.getByRole("button", { name: /بدء جلسة|Start/i }).first();
		const hasCourse = await startBtn.isVisible({ timeout: 5_000 }).catch(() => false);
		if (!hasCourse) {
	...
	test("ends an active session", async ({ page }) => {
		...
		await startBtn.click();
		await page.waitForSelector('button:has-text("إنهاء الجلسة")', { timeout: 10_000 });
		await page.getByRole("button", { name: /إنهاء الجلسة/ }).click();
		await expect(page.getByText("المواد الدراسية").first()).toBeVisible({ timeout: 8_000 });
	});
