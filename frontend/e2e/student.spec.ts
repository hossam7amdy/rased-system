...
import { studentTest as test } from "./fixtures/auth.ts";
import { API_BASE, CREDS } from "./global-setup.ts";
...
test.describe("Student dashboard", () => {
  ...
  test("course selector is visible", async ({ page }) => {
    const selectVisible = await page
      .locator("#course-select")
      .isVisible({ timeout: 5_000 })
      .catch(() => false);
    const emptyVisible = await page
      .getByText(/لا توجد مواد/)
  ...
  test("QR scan simulated via API marks attendance", async ({ page }) => {
    ...
    if (!studentToken) {
      test.skip(true, "student token missing from localStorage");
      return;
    }
    ...
    const loginRes = await fetch(`${API_BASE}/auth/login`, {
      ...
      body: JSON.stringify({
        email: CREDS.professor.email,
        password: CREDS.professor.password,
      }),
    });
    if (!loginRes.ok) {
      test.skip(true, "professor login failed");
      return;
    }
    ...
    if (!sessionRes.ok) {
      test.skip(true, "could not start attendance session");
      return;
    }
    ...
    try {
      ...
      const qrRes = await fetch(
        `${API_BASE}/attendance/current-qr/${courseId}`,
        {
          headers: { Authorization: `Bearer ${profToken}` },
        },
      );
